...
namespace SteamEngine {
	...
	public sealed class Utility {
		...
		*/
		...
		//used in the TagHolder copying constructor, and copying constructors
		//generated by ClassTemplate
		public static object CopyTagValue(object copyFrom) {
			//if the object is some collection, we should copy it.
			if (copyFrom.GetType() == typeof(ArrayList)) {
				ArrayList original = (ArrayList)copyFrom;
				ArrayList newList = new ArrayList(original.Capacity);
				foreach (object item in original) {
					newList.Add(CopyTagValue(item));//recursion
				}
				return newList;
			} else if (copyFrom.GetType() == typeof(Hashtable)) {
				Hashtable original = (Hashtable)copyFrom;
				Hashtable newTable = new Hashtable();//HashTable has no Capacity property :\
				foreach (DictionaryEntry entry in original) {
					newTable[entry.Key]=CopyTagValue(entry.Value);//recursion
				}
				return newTable;
			}

			ICloneable cloneable = copyFrom as ICloneable;
			if (cloneable != null) {
