	...
	public class ScriptedCommands : CompiledScript {	//Our script must extend 'Script'
		...
		public void Remove(GameConn c, IPoint3D targetted, object targData) {
			...
			if (targ == null) {
				...
				c.CurCharacter.SysMessage("Zamer item nebo character");
				c.CurCharacter.Conn.Target(false, remove, null, null);
			}
			targ.Delete();
		}
		...
		public void def_remove(AbstractCharacter self) {
			...
			//remove - the function delegate to be used (called)
			//second null - no function is to be called when targetting was canceled
