...
namespace SteamEngine.CompiledScripts {
	...
	public class MagerySkillDef : SkillDef {
		...
		protected override void On_Select(Character ch) {
			SpellBook book = ch.currentSkillTarget2 as SpellBook;
			SpellDef spell = (SpellDef) ch.currentSkillParam1;
			if (book != null) {
				if ((book.TopObj() == ch) && book.HasSpell(spell)) {
					spell.Trigger_Select(ch);
					return;
				}
			}

			ch.WriteLine("Casting without book not implemented yet");
			//TODO scrolls
		}
