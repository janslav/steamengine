...
namespace Jolt
{
    ...
    public sealed class XmlDocCommentReaderSettings : ConfigurationSection
    {
        ...
        private static XmlDocCommentReaderSettings CreateDefaultSettings()
        {
            ...
            string netfxSubDirectoryName = Path.Combine("Microsoft.NET", framework);
            string referenceAssembliesSubDirectoryName = Path.Combine("Reference Assemblies", Path.Combine("Microsoft", framework));
            string programFilesDirectoryName = Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles);

            string netfxDirectoryName = Path.Combine(Path.GetDirectoryName(Environment.SystemDirectory), netfxSubDirectoryName);
            string referenceAssembliesDirectoryNameX86 = Path.Combine(programFilesDirectoryName, referenceAssembliesSubDirectoryName);
            string referenceAssembliesDirectoryName = Path.Combine(programFilesDirectoryName + " (x86)", referenceAssembliesSubDirectoryName);

            string culture = CultureInfo.CurrentCulture.TwoLetterISOLanguageName;
            string netfx3_0 = Path.Combine("3.0", culture);
            string netfx3_5 = "3.5";

            return new XmlDocCommentReaderSettings(new string[]
            {
