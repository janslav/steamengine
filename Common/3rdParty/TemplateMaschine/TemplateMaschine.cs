...
namespace TemplateMaschine
{
    ...
    public class Template
	{
        ...
        private ArrayList usings = new ArrayList();
		...
		private void ParseTemplateAssignment()
		{
			while(true)
			{
				TokenInfo tokenInfo = NextToken();
				if (tokenInfo.token != Token.RTag)
				{
		...
		private void ParseTemplate(string templateFile)
		{
			...
			TokenInfo lastTokenInfo = new TokenInfo(Token.Eof,null);
			while(true)
			{
				TokenInfo tokenInfo = NextToken();
			...
			using (StreamReader sr = new StreamReader(fileStream))
			{
				...
				while((s = sr.ReadLine()) != null)
				{
					if (s.IndexOf("Include") != -1)
					{
                        ...
                        string resourceName = FindResourceName(resourceNames, fileName);
        ...
        private string FindResourceName(string[] resourceNames, string filename) {
        	foreach (string resName in resourceNames) {
        		if (resName.EndsWith(filename)) {
        			return resName;
        		}
        	...
        	return null;
        	//Array.Find<string>(resourceNames, delegate(string fn) { return fn.EndsWith(fileName); });
        ...
        private void ProcessTemplate(string templateFile, string fileNameForDebugging)
		{
			...
			{
				resultString.Insert(0, "this." + arg.name + " = (" + arg.type + ")args[" + argNo + "];\r\n");
				argNo++;
			}
			...
			{
				resultString.Insert(0, "private " + argument.type + " " + argument.name + ";\r\n");
			}
            ...
            if (!StringIsNullOrEmpty(fileNameForDebugging))
            {
